...
import { slidingWindowCounterLuaMirror, type SlidingWindowCounterLuaState } from './lua-runner.js';

let luaFound = false;
try {
  const mod = await import('../../src/stores/redis.js');
  luaFound = typeof mod.slidingWindowCounterLua === 'string';
} catch {
  luaFound = false;
}
...
if (luaFound) {
  describe('Sliding Window Counter Lua conformance', () => {
    it('matches Lua decisions over randomized timelines', () => {
      fc.assert(
        fc.property(
          ...
          (limit, windowMs, costs, advances) => {
            const clock = new ManualClock(1_000_000);
            const jsStrategy = createSlidingCounterStrategy({ limit, windowMs, clock });

            let luaState: SlidingWindowCounterLuaState | null = null;
            ...
            for (let i = 0; i < len; i++) {
              ...
              const luaOutcome = slidingWindowCounterLuaMirror(luaState, now, cost, limit, windowMs);
