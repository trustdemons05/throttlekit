...
import { fixedWindowLuaMirror, type FixedWindowLuaState } from './lua-runner.js';

let luaFound = false;
try {
  const mod = await import('../../src/stores/redis.js');
  luaFound = typeof mod.fixedWindowLua === 'string';
} catch {
  luaFound = false;
}
...
if (luaFound) {
  describe('Fixed Window Lua conformance', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (limit, windowMs, costs, advances) => {
            ...
            const jsStrategy = createFixedWindowStrategy({ limit, windowMs, clock });

            let luaState: FixedWindowLuaState | null = null;
            ...
            for (let i = 0; i < len; i++) {
              ...
              const luaOutcome = fixedWindowLuaMirror(luaState, now, cost, limit, windowMs);
