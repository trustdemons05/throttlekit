...
import { slidingWindowLogLuaMirror } from './lua-runner.js';

let luaFound = false;
try {
  const mod = await import('../../src/stores/redis.js');
  luaFound = typeof mod.slidingWindowLogLua === 'string';
} catch {
  luaFound = false;
}
...
if (luaFound) {
  describe('Sliding Window Log Lua conformance', () => {
      ...
      fc.assert(
        fc.property(
          ...
          (limit, windowMs, costs, advances) => {
            ...
            const jsStrategy = createSlidingLogStrategy({ limit, windowMs, clock });
            ...
            for (let i = 0; i < len; i++) {
              ...
              const luaOutcome = slidingWindowLogLuaMirror(luaState, now, cost, limit, windowMs);
